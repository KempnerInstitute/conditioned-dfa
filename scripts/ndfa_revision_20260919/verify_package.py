"""Validate a fresh review extraction outside the workspace."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='ndfa_revised_review_') as temp:
        ...
        commands={
            'retained_evidence':[sys.executable,'-B','reproduce.py','--checks','integrity,statistics,legacy,focused,paper,smoke'],
            'revision_evidence':[sys.executable,'-B','verify_revision.py'],
            'revision_math':[sys.executable,'-B','-m','pytest','-q','-p','no:cacheprovider','tests/test_ndfa_revision_math.py'],
            'second_review_statistics':[sys.executable,'-B','scripts/ndfa_revision_20260919/verify.py'],
            'revised_figures':[sys.executable,'-B','scripts/ndfa_revision_20260919/redraw.py']}
        ...
        import fitz
        plotted=json.loads((Path(temp)/'figure_previews/manifest.json').read_text())
        for name in plotted:
        ...
        results['revised_figures']['pixel_identical_assets']=len(plotted)
    ...
    results.update(accepted=True,fresh_extraction_outside_workspace=True,model_training=False,new_test_evaluation=False)
