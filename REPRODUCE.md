# Reproducing the September 18–19 revision

The [artifact map](docs/research/ndfa_revision_artifact_map_20260919.json) binds
this revision to exact source and evidence hashes. The manuscript and code
repositories share the tag `ndfa-revision-2026-09-19`. The main ICLR text,
including every main figure and table, fits within nine pages in the official
style. The full documents have 56 pages (ICLR) and 54 pages (author preprint),
with 17 active figures and 59 references.
...
```bash
git clone --branch ndfa-revision-2026-09-19 https://github.com/houman1359/Info-DFA-draft.git drafts/Info-DFA
...
corrected calculation. The archive retains every planned case; large finite
...
```bash
NDFA_FIGURE_CACHE=revision/figure_cache.pkl NDFA_FIGURE_OUTPUT=/tmp/ndfa_figure_previews NDFA_PAPER_FIGURES=/tmp/ndfa_figure_pdfs python scripts/ndfa_revision_20260918/redraw.py
python analysis/validate_mode_timing.py
```

The first command produces 13 revised plots; the second produces the corrected
