...
def state_keys(model):
    return STATE_KEYS + (("decor_weights", "decor_means") if hasattr(model, "decorators") else ())


def state_tensors(model):
    return {key: [value.detach().cpu().clone() for value in getattr(model, key)] for key in state_keys(model)}
...
def model_fingerprint(model):
    return {"training": model.training, "bn_eps": model.bn_eps, "bn_momentum": model.bn_momentum,
            "tensors": {key: [tensor_digest(value) for value in getattr(model, key)] for key in state_keys(model)},
            "decorrelation_updates": list(getattr(model,"decorrelation_updates",[])),
            "decor_cache": [{"matrix":tensor_digest(c.matrix),"mean":tensor_digest(c.mean),"training":c.training,"input_shape":list(c.input_shape)} for c in getattr(model,"_decor_cache",[])],
...
def restore_model(checkpoint, device="cpu"):
    ...
    if config.get("decorrelation") is not None:
        from model import DecorrelatedMLP
        spec = config["decorrelation"]
        model = DecorrelatedMLP.from_base(model, spec["learning_rate"], sample_fraction=spec["sample_fraction"], mean_momentum=spec["mean_momentum"])
        for layer, matrix, mean in zip(model.decorators, checkpoint["state"]["decor_weights"], checkpoint["state"]["decor_means"]):
            layer.decor_weight = matrix.to(device).clone()
            layer.running_mean = mean.to(device).clone()
        model.decorrelation_updates = list(checkpoint["decorrelation_updates"])
...
def export_final(model, data, args, endpoint, output):
    ...
    checkpoint = {"schema_version": 2, "stage": "final validation-only inference state",
                  ...
                  "decorrelation": model.decorrelation_spec() if hasattr(model, "decorators") else None},
                  "decorrelation_updates": getattr(model, "decorrelation_updates", []),
    ...
    receipt = {"method": endpoint["method"], "seed": endpoint["seed"], "step": endpoint["step"],
               ...
               "preprocessing_tensor_sha256": None if preprocessing is None else {key:tensor_digest(preprocessing[key]) for key in ("channel_mean","channel_std")},
...
def audit_saved_case(output, command, recorded_endpoint, recorded_standardization):
    ...
    require(state["model"] == {"input_dim": 3072, "hidden_dims": command["hidden_dims"], "output_dim": 10,
                               "batchnorm": True, "bn_eps": 1e-5, "bn_momentum": .1, "decorrelation": command["decorrelation"]}, "saved architecture changed")
    ...
    if command["decorrelation"] is not None:
        expected_shapes.update(decor_weights=[(d,d) for d in expected_dims[:-1]], decor_means=[(d,) for d in expected_dims[:-1]])
        require(state["decorrelation_updates"] == [command["steps"]]*3, "decorator update count mismatch")
    else:
        require(state["decorrelation_updates"] == [], "unexpected decorator state")
    require(set(state["state"]) == set(expected_shapes), "unexpected checkpoint tensor inventory")
    require(state["schema_version"] == 2, "wrong inference checkpoint schema")
    ...
    require(receipt["preprocessing_tensor_sha256"] == {key:tensor_digest(state["preprocessing"][key]) for key in ("channel_mean","channel_std")}, "preprocessing hash mismatch")
...
@torch.no_grad()
def inference_logits(model, uint8_images, checkpoint, *, batch_size=256):
    """Authoritative frozen inference helper; no dataset loading or test access.

    Input is NCHW uint8, matching the frozen unaugmented validation pipeline.
    Caller chooses identities. All learned tensors, cached BN state and global
    RNG are preserved; output is a CPU tensor.
    """
    require(uint8_images.dtype == torch.uint8 and uint8_images.ndim == 4 and
            tuple(uint8_images.shape[1:]) == (3,32,32), "inference requires NCHW CIFAR uint8")
    before = model_fingerprint(model)
    before_rng = rng_fingerprint(model.device)
    training = model.training
    pre = checkpoint["preprocessing"]
    require(pre["validation_used"] is False and pre["uint8_scale"] == 255., "invalid inference preprocessing")
    outputs = []
    try:
        model.training = False
        with legacy.preserve_batchnorm_state(model):
            for start in range(0,len(uint8_images),batch_size):
                x = uint8_images[start:start+batch_size].float()/255.
                x = (x-pre["channel_mean"].reshape(1,3,1,1))/pre["channel_std"].reshape(1,3,1,1)
                outputs.append(model.forward(x.flatten(1).to(model.device))[0].cpu())
    finally:
        model.training = training
    require(model_fingerprint(model) == before and rng_fingerprint(model.device) == before_rng, "inference mutated state/RNG")
