...
The historical cell-level paired tests treated the 32 designed cells of each regime as
...
script re-does the inference at the data-seed level:
  ...
  3. Descriptive whole-seed percentile bootstrap 95% interval for the mean delta:
     resample global seeds, preserving their entire cell vectors
     (all 5^5 draws; designed cells held fixed).
...
Outputs (all under ``results/infodfa_seedlevel_stats_corrected_20260918``):
...
OUT = Path(os.environ.get("NDFA_SEED_STATS_OUTPUT", RESULTS / "infodfa_seedlevel_stats_corrected_20260918"))
...
def whole_seed_bootstrap_ci(
    delta: pd.DataFrame, *, rng: np.random.Generator, n_boot: int = N_BOOT
) -> tuple[float, float]:
    """Percentile interval over whole global seeds; designed grid held fixed.

    Five seeds offer limited information about coverage. All cells belonging to
    a selected seed move together; cells are not additional independent draws.
    """
    mat = delta.to_numpy()
    seed_means = mat.mean(axis=0)
    import itertools
    if len(seed_means) <= 7:
        boot = np.array([np.mean(draw) for draw in itertools.product(seed_means, repeat=len(seed_means))])
    else:
        indices = rng.integers(0, len(seed_means), size=(n_boot, len(seed_means)))
        boot = seed_means[indices].mean(axis=1)
...
def seedlevel_tests(val: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for cond in COND_ORDER:
        ...
        for method, reference in COMPARISONS:
            ...
            ci_low, ci_high = whole_seed_bootstrap_ci(mat, rng=rng)
            rows.append(
                {
                    "condition": cond,
                    "method": method,
                    ...
                    "seed_boot_ci_low_pp": ci_low,
                    "seed_boot_ci_high_pp": ci_high,
                    "t_stat": float(t_stat),
                    "t_p": float(t_p),
...
def write_markdown(tests: pd.DataFrame, path: Path) -> None:
    lines = [
        ...
        "seed) at fixed full-rank feedback; the interval is a whole-seed percentile bootstrap",
        f"(fixed cells, all 5^5 global-seed draws; limited coverage information at n=5). With n=5 seeds the",
        "smallest attainable Wilcoxon p is 0.0625 (two-sided, all deltas same sign).",
        "",
        "| regime | comparison | mean delta (pp) | seed range (pp) | 95% seed bootstrap interval (pp) | t | t-p | Wilcoxon-p |",
        "|---|---|---:|---|---|---:|---:|---:|",
    ]
    for _, r in tests.iterrows():
        lines.append(
            ...
            f"[{r['seed_boot_ci_low_pp']:+.2f}, {r['seed_boot_ci_high_pp']:+.2f}] | "
