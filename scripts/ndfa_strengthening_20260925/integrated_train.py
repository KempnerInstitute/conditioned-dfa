"""Integrated validation-development and joint-confirmation training with validation checkpoints and work budgets.
...
def args_for(config,case,seed,output,device):
    args=base.parse_args(['--output-dir',str(output),'--data-dir',config.get('data_dir','unused'),
        '--hidden-dims',*map(str,case.get('hidden_dims',config['hidden_dims'])),'--device',device,'--threads',str(config['threads']),
...
def train(config,case,seed,output,device='cuda',stop_after_step=None):
    ...
    data=component.load_data(config,case,seed,args);model,feedback=nr.make_model(config,case,args,data)
    ...
    if config.get('branch_from') and not checkpoint.exists():
        component.initialize_branch(config,case,seed,out,digest)
    if checkpoint.exists():
        ...
        conditioned_steps=saved['conditioned_steps'];last_checkpoint_work=work;flop_counts=saved.get('flop_counts',{})
    ...
    def save(path=None):
        nr.atomic_save(dict(**model_payload(),optimizer=opt.state_dict(),foof=foof.state_dict() if foof else None,
            ...
            best_payload=torch.load(out/'best.pt',map_location='cpu',weights_only=False),conditioned_steps=conditioned_steps,flop_counts=flop_counts),path or checkpoint)
    def record():
        ...
        if case.get('error_operator') and progress()<.25:
            row['error_direction']=component.direction_probe(model,feedback,data,args,case,seed)
        if config.get('representation_probes'):
            row['representation']=component.representation_probe(model,data,args,config,case,seed)
        history.append(row)
    ...
    try:
        if not checkpoint.exists():
            if foof:
                ...
                component.add_flops(flop_counts,component.calibration_flops(model,args.batch_size,config['foof_calibration_batches']))
            record();save()
        while (work<budget if is_work else step<total):
            ...
            before_progress=progress();selected=component.active_case(case,before_progress)
            ...
            update=component.gradient(model,feedback,x,y,args,selected,foof)
            ...
            component.add_flops(flop_counts,component.step_flops(model,selected,len(idx),foof_step=foof.step-1 if foof else None,inverse_period=config['foof_inverse_period']))
            ...
            if not is_work and any(step==int(f*total) for f in config.get('branch_fractions',[])):
                save(out/f'branch_{step}.pt')
    ...
    result=dict(config_hash=digest,case=case,seed=seed,status=status,error=error,epoch=epoch,completed_updates=step,
        ...
        last_step_seconds=final_step_seconds,attempts=attempt+1,flops=flop_counts,flop_scope=component.FLOP_SCOPE,training_examples=len(data.train),validation_examples=len(data.validation),peak_memory_bytes=torch.cuda.max_memory_allocated() if device=='cuda' else None)
...
def main():
    ...
    config=component.task_config(config,case,seed)
